...
internal static class LogManager
{
    ...
    public static void Error(string message, ConsoleColor color = ConsoleColor.Red)
    {
        ...
        Logger.Raw($"[ERROR] [{SerpentsHand.Singleton.Name}] {message}", color);
    }

    internal static (string logResult, bool success) GetLogHistory()
    {
        ...
        if (SerpentsHand.Singleton.Config?.ShRole != null)
        {
            stringBuilder.AppendLine("\n--- Serpent's Hand CustomRole ---\n");
            stringBuilder.Append($"{YamlConfigParser.Serializer.Serialize(SerpentsHand.Singleton.Config.ShRole)}");
        }
